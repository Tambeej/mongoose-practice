...
mongoose
  .connect(
    "mongodb+srv://bejella_db_user:Rm9S!VeT*8$fPrR@cluster0.7qwzo1r.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0/mongoose-practice",
    {
...
Book.find({ pages: { $lt: 500, $gt: 200 } }).then(function (books) {
  console.log("Books with 200 < pages < 500:", books);
});
...
Book.find({ rating: { $lt: 5 } })
  ...
  .then(function (books) {
    console.log(
      "books whose rating is less than 5, and sort by the author's name:",
      books
    );
  });
...
Book.find({ genres: "Fiction" })
  ...
  .then(function (books) {
    console.log(
      "Fiction books, skip the first 2, and display only 3 of them:",
      books
    );
  });
